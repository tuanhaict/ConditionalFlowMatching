...
flags.DEFINE_string("resume_from_checkpoint", None, help="Đường dẫn tới file checkpoint (ví dụ: './results/otcfm/otcfm_cifar10_weights_step_20000.pt')")
# Evaluation
flags.DEFINE_integer(
    "save_step",
    20000,
...
def train(argv):
    ...
    if FLAGS.resume_from_checkpoint:
        if os.path.exists(FLAGS.resume_from_checkpoint):
            print(f"Đang load checkpoint: {FLAGS.resume_from_checkpoint}")
            checkpoint = torch.load(FLAGS.resume_from_checkpoint, map_location=device)
